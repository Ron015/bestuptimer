console.log("Welcome to Uptime Ron");
...
let hostname = process.env.UPTIME_RON_HOST || args.host;
...
const port = parseInt(process.env.UPTIME_RON_PORT || process.env.PORT || args.port || 3001);

// SSL
const sslKey = process.env.UPTIME_RON_SSL_KEY || process.env.SSL_KEY || args["ssl-key"] || undefined;
const sslCert = process.env.UPTIME_RON_SSL_CERT || process.env.SSL_CERT || args["ssl-cert"] || undefined;
const disableFrameSameOrigin = !!process.env.UPTIME_RON_DISABLE_FRAME_SAMEORIGIN || args["disable-frame-sameorigin"] || false;
        ...
        socket.on("setup", async (username, password, callback) => {
            try {
                ...
                if ((await R.count("user")) !== 0) {
                    throw new Error("Uptime Ron has been initialized. If you want to run setup again, please delete the database.");
                }
...
async function initDatabase(testMode = false) {
    ...
    // If there is no record in user table, it is a new Uptime Ron instance, need to setup
    if ((await R.count("user")) === 0) {
